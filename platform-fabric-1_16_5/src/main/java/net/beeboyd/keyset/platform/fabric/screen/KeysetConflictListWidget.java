...
public final class KeysetConflictListWidget
      ...
      entries.add(
          new GroupEntry(
              ...
              groupMode == KeysetConflictGroupMode.BY_KEY
                  ? translatable("keyset.group.key", group.getTitle(), group.getConflictCount())
                  : translatable(
  ...
  public void showPreview(AutoResolvePlan previewPlan) {
    ...
    if (previewPlan.getChanges().isEmpty()) {
      entries.add(new PreviewEntry(this, translatable("keyset.resolve.no_preview")));
    } else {
  ...
  private static final class GroupEntry extends Entry {
    ...
    public void render(
        ...
        boolean hovered,
        float tickDelta) {
      DrawableHelper.fill(matrices, x, y, x + entryWidth, y + entryHeight - 1, 0x6A1C2330);
      DrawableHelper.drawTextWithShadow(
          matrices, owner.client.textRenderer, text, x + 6, y + 8, 0xE8D7A0);
    }
  }
  ...
  private static final class DetailEntry extends Entry {
    ...
    public void render(
        ...
        float tickDelta) {
      DrawableHelper.drawTextWithShadow(
          matrices, owner.client.textRenderer, text, x + 12, y + 8, 0xB8C7D9);
    }
  }
  ...
  private static final class BindingEntry extends Entry {
    ...
    public void render(
      ...
      if (selected) {
        DrawableHelper.fill(matrices, x, y, x + entryWidth, y + entryHeight - 1, 0x7A304A68);
        drawBorder(matrices, x, y, entryWidth, entryHeight - 1, 0xFF8AB3D6);
      } else if (hovered) {
        DrawableHelper.fill(matrices, x, y, x + entryWidth, y + entryHeight - 1, 0x4A28313A);
      }

      DrawableHelper.drawTextWithShadow(
          matrices,
          owner.client.textRenderer,
          literal(bindingDescriptor.getDisplayName()),
          x + 8,
          y + 5,
          0xF2F5F8);
      DrawableHelper.drawTextWithShadow(
  ...
  private static final class PreviewEntry extends Entry {
    ...
    public void render(
        ...
        float tickDelta) {
      DrawableHelper.drawTextWithShadow(
          matrices, owner.client.textRenderer, text, x + 6, y + 8, 0xE0E6ED);
    }
  }

  private static void drawBorder(
      MatrixStack matrices, int x, int y, int width, int height, int color) {
