...
public final class KeysetConflictListWidget
    extends ContainerObjectSelectionList<KeysetConflictListWidget.Entry> {
  ...
  public KeysetConflictListWidget(
      Minecraft client, int width, int top, int bottom, int rowHeight, Listener listener) {
  ...
  public void showPreview(AutoResolvePlan previewPlan) {
    ...
    if (previewPlan.getChanges().isEmpty()) {
      entries.add(
          new PreviewEntry(this, Component.translatable("keyset.resolve.no_preview"), false));
    } else {
  ...
  Component fitText(Component text, int maxWidth) {
    return Component.literal(ellipsize(text.getString(), maxWidth));
  }

  Component fitText(String text, int maxWidth) {
    return Component.literal(ellipsize(text, maxWidth));
  }
  ...
  int singleLineTextY(int y, int entryHeight) {
    return y + Math.max(2, (entryHeight - minecraft.font.lineHeight) / 2);
  }

  int bindingNameY(int y, int entryHeight) {
    int lineGap = compactRows ? 1 : 2;
    int totalTextHeight = (minecraft.font.lineHeight * 2) + lineGap;
    return y + Math.max(2, (entryHeight - totalTextHeight) / 2);
  }

  int bindingMetaY(int nameY) {
    return nameY + minecraft.font.lineHeight + (compactRows ? 1 : 2);
  }
  ...
  int badgeTextY(int badgeY) {
    return badgeY + Math.max(1, (badgeHeight() - minecraft.font.lineHeight) / 2);
  }
  ...
  private String ellipsize(String value, int maxWidth) {
    if (value == null || minecraft.font.width(value) <= maxWidth) {
      return value == null ? "" : value;
    }

    String ellipsis = "...";
    String candidate = value;
    while (!candidate.isEmpty() && minecraft.font.width(candidate + ellipsis) > maxWidth) {
  ...
  private static final class GroupEntry extends Entry {
    ...
    private GroupEntry(KeysetConflictListWidget owner, Component text, String firstBindingId) {
      super(owner);
      this.text = text;
    ...
    public void extractContent(
        GuiGraphicsExtractor context, int mouseX, int mouseY, boolean hovered, float tickDelta) {
      int x = getX();
      int y = getY();
  ...
  private static final class DetailEntry extends Entry {
    ...
    private DetailEntry(KeysetConflictListWidget owner, Component text, String firstBindingId) {
      super(owner);
      this.text = text;
      this.firstBindingId = firstBindingId;
    }
  ...
  private static final class BindingEntry extends Entry {
    ...
    public void extractContent(
      ...
      if (borderColor != 0) {
        context.outline(x, y + 2, entryWidth, entryHeight - 4, borderColor);
      }

      String keyLabel = bindingDescriptor.getKeyDisplayName();
      int badgeWidth =
          clamp(
              owner.minecraft.font.width(keyLabel) + 14,
              40,
      ...
      context.outline(badgeX, badgeY, badgeWidth, badgeHeight, keyBadgeBorder);
  ...
  private static final class PreviewEntry extends Entry {
    ...
    private PreviewEntry(KeysetConflictListWidget owner, Component text, boolean summary) {
      super(owner);
      this.text = text;
