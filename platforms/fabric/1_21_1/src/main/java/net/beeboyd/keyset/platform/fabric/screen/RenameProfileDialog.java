...
public final class RenameProfileDialog extends Screen {
  ...
  protected void init() {
    ...
    nameField = new TextFieldWidget(textRenderer, px + 10, py + 30, PW - 20, 18, Text.empty());
    ...
    addDrawableChild(
        ButtonWidget.builder(Text.translatable("keyset.action.save"), b -> save())
            .dimensions(px + PW - 90, py + PH - 26, 80, 18)
            .build());
    addDrawableChild(
        ButtonWidget.builder(
                Text.translatable("keyset.action.cancel"), b -> client.setScreen(parent))
            .dimensions(px + 10, py + PH - 26, 70, 18)
            .build());
  }
