...
public final class KeysetTextFieldWidget extends TextFieldWidget {
  ...
  public void renderWidget(DrawContext ctx, int mouseX, int mouseY, float delta) {
    ...
    ctx.drawStrokedRectangle(getX(), getY(), getWidth(), getHeight(), border);
    if (isFocused()) {
    ...
    try {
      ctx.getMatrices().translate((float) TEXT_PAD_X, (float) textYOffset());
      super.renderWidget(ctx, mouseX - TEXT_PAD_X, mouseY, delta);
    } finally {
  ...
  public void onClick(Click click, boolean doubleClick) {
    super.onClick(new Click(click.x() - TEXT_PAD_X, click.y(), click.buttonInfo()), doubleClick);
  }
