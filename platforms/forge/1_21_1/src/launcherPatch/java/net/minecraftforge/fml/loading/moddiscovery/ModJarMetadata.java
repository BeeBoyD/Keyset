...
public final class ModJarMetadata implements JarMetadata {
  ...
  public void setModFile(IModFile file) {
    ...
    var automaticModuleName =
        jar.moduleDataProvider().getManifest().getMainAttributes().getValue(AUTOMATIC_MODULE_NAME);
    // Forge 1.21.1 userdev under Loom builds the Minecraft system mod from a filtered view of the
    // merged Forge jar. Preserve the logical minecraft module name instead of inheriting Forge's
    // manifest-level Automatic-Module-Name, or the game layer ends up with two forge modules.
    if (automaticModuleName != null && !"minecraft".equals(this.name)) {
      this.name = automaticModuleName;
    }
  }
  ...
  public ModuleDescriptor descriptor() {
    ...
    for (var provider : modFile.getSecureJar().getProviders()) {
      ...
      builder.provides(provider.serviceName(), provider.providers());
    }
