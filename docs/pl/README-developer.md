# Keyset — Dokumentacja deweloperska

Ten dokument opisuje architekturę, układ modułów, system build i workflow kontrybucji w Keyset.

Opis dla graczy znajduje się w [głównym README](../../README.md).

---

## Architecture

Keyset to wielomodułowy projekt Gradle. Cała logika biznesowa znajduje się w `modules/core`: czyste Java bez importów Minecraft ani loader. Platformowe entrypoints są cienkimi wrappers.
...
Buildy działają w GitHub Actions. Nie uruchamiaj pełnych buildów workspace lokalnie, chyba że debugujesz system build.
...
Dodaj tag `[build:<label>]` do wiadomości commit:
...
Wymaga `MODRINTH_TOKEN` jako zmiennej środowiskowej albo `modrinthToken` w `gradle-local.properties` (gitignored).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` używają `jar` zamiast `remapJar` z powodu konfliktów mappings. Zobacz `jarTaskOverrides` w głównym `build.gradle`.
...
26.1 to pierwsza nieobfuskowana wersja Minecraft. Yarn nie jest używany.
...
Linia `1.0.x` otrzymuje tylko krytyczne poprawki.
Share, Auto-Switch i UI 2.0 nie są backportowane.
...
Profile są przechowywane w `config/keybindprofiles.json`. Format jest stabilny w obsługiwanej rodzinie release. Nieznane keybind ID są zachowywane.

---

## i18n

Pliki lang znajdują się w:
