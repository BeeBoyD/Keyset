# Keyset — Довідник розробника

Цей документ описує архітектуру, структуру модулів, систему build і процес внесення змін у Keyset.

Огляд для гравців дивіться в [кореневому README](../../README.md).

---

## Architecture

Keyset — це багатомодульний Gradle-проєкт. Уся бізнес-логіка живе в `modules/core`: чиста Java без імпортів Minecraft або loader. Платформні entrypoints — тонкі wrappers.
...
Build-и виконуються в GitHub Actions. Не запускайте повні workspace build-и локально, якщо не налагоджуєте систему build.
...
Додайте тег `[build:<label>]` у повідомлення commit:
...
Потрібен `MODRINTH_TOKEN` як змінна середовища або `modrinthToken` у `gradle-local.properties` (gitignored).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` використовують `jar` замість `remapJar` через конфлікти mappings. Див. `jarTaskOverrides` у кореневому `build.gradle`.
...
26.1 — перша необфускована версія Minecraft. Yarn не використовується.
...
Лінія `1.0.x` отримує лише критичні виправлення.
Share, Auto-Switch і UI 2.0 не backport-яться.
...
Профілі зберігаються в `config/keybindprofiles.json`. Формат стабільний у підтримуваній release family. Невідомі keybind ID зберігаються.

---

## i18n

Lang-файли розташовані в:
