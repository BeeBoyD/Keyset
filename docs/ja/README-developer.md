...
このドキュメントは Keyset のアーキテクチャ、モジュール構成、build システム、貢献フローを説明します。

プレイヤー向け概要は [root README](../../README.md) を参照してください。

---

## Architecture

Keyset は Gradle のマルチモジュールプロジェクトです。ビジネスロジックはすべて `modules/core` にあり、Minecraft や loader の import を持たない純粋な Java です。プラットフォーム entrypoint は薄い wrapper だけです。
...
Build は GitHub Actions で実行されます。build システムをデバッグする場合以外、ローカルで workspace 全体の build を実行しないでください。
...
commit message に `[build:<label>]` タグを追加します:
...
環境変数 `MODRINTH_TOKEN`、または `gradle-local.properties` の `modrinthToken`（gitignored）が必要です。

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` mapping の競合により `remapJar` ではなく `jar` を使います。root `build.gradle` の `jarTaskOverrides` を参照してください。
...
26.1 は最初の非難読化 Minecraft バージョンです。Yarn は使いません。
...
`1.0.x` リリースラインは重要な bug fix のみ受け取ります。
Share、Auto-Switch、2.0 UI は backport されません。
...
プロファイルは `config/keybindprofiles.json` に保存されます。形式はサポート対象の release family で安定しています。不明な keybind ID は保持されます。
