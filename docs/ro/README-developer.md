# Keyset — Referință pentru dezvoltatori

Acest document acoperă arhitectura, organizarea modulelor, sistemul de build și fluxul de contribuție pentru Keyset.

Pentru prezentarea destinată jucătorilor, vezi [README-ul principal](../../README.md).

---

## Architecture

Keyset este un proiect Gradle multi-modul. Toată logica de business se află în `modules/core`: Java pur, fără importuri Minecraft sau loader. Entrypoint-urile de platformă sunt doar wrappers subțiri.
...
Build-urile rulează în GitHub Actions. Nu rula build-uri complete local decât dacă depanezi sistemul de build.
...
Adaugă o etichetă `[build:<label>]` în mesajul commitului:
...
Necesită `MODRINTH_TOKEN` ca variabilă de mediu sau `modrinthToken` în `gradle-local.properties` (ignorat de git).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` folosesc `jar` în loc de `remapJar` din cauza conflictelor de mappings. Vezi `jarTaskOverrides` în `build.gradle` din rădăcină.
...
26.1 este prima versiune Minecraft neobfuscată. Yarn nu este folosit.
...
Linia `1.0.x` primește doar corecții critice.
Share, Auto-Switch și UI-ul 2.0 nu sunt backportate.
...
Profilurile sunt stocate în `config/keybindprofiles.json`. Formatul este stabil în familia de release-uri suportate. ID-urile keybind necunoscute sunt păstrate.

---

## i18n

Fișierele lang se află în:
