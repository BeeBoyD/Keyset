# Keyset — Ontwikkelaarsreferentie

Dit document beschrijft architectuur, module-indeling, build-systeem en bijdrageworkflow voor Keyset.

Voor het spelersoverzicht zie de [root README](../../README.md).

---

## Architecture

Keyset is een Gradle multi-module project. Alle businesslogica staat in `modules/core`: pure Java zonder Minecraft- of loader-imports. Platform-entrypoints zijn dunne wrappers.
...
Builds draaien in GitHub Actions. Draai lokaal geen volledige workspace-builds tenzij je het build-systeem debugt.
...
Voeg een `[build:<label>]` tag toe aan het commitbericht:
...
Vereist `MODRINTH_TOKEN` als environment variable of `modrinthToken` in `gradle-local.properties` (gitignored).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` gebruiken `jar` in plaats van `remapJar` door mappingconflicten. Zie `jarTaskOverrides` in root `build.gradle`.
...
26.1 is de eerste niet-geobfusceerde Minecraft-versie. Yarn wordt niet gebruikt.
...
De `1.0.x` releaselijn krijgt alleen kritieke bugfixes.
Share, Auto-Switch en de 2.0 UI worden niet gebackport.
...
Profielen worden opgeslagen in `config/keybindprofiles.json`. Het formaat is stabiel binnen de ondersteunde releasefamilie. Onbekende keybind-ID’s blijven behouden.

---

## i18n

Lang-bestanden staan in:
