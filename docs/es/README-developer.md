# Keyset — Referencia para desarrolladores

Este documento cubre la arquitectura, el diseño de módulos, el sistema de build y el flujo de contribución de Keyset.

Para la descripción general para jugadores, consulta el [README raíz](../../README.md).

---

## Architecture

Keyset es un proyecto Gradle multimódulo. Toda la lógica de negocio vive en `modules/core`: Java puro sin imports de Minecraft ni de loaders. Los entrypoints de plataforma son wrappers delgados.
...
Los builds se ejecutan en GitHub Actions. No ejecutes builds completos del workspace localmente salvo que estés depurando el sistema de build.
...
Añade una etiqueta `[build:<label>]` al mensaje del commit:
...
Requiere `MODRINTH_TOKEN` como variable de entorno o `modrinthToken` en `gradle-local.properties` (ignorado por git).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` usan `jar` en vez de `remapJar` por conflictos de mappings. Consulta `jarTaskOverrides` en el `build.gradle` raíz.
...
26.1 es la primera versión de Minecraft sin obfuscación. Yarn no se usa.
...
La línea `1.0.x` recibe solo correcciones críticas.
Share, Auto-Switch y la UI 2.0 no se backportean.
...
Los perfiles se guardan en `config/keybindprofiles.json`. El formato es estable en la familia de releases compatibles. Los IDs de keybind desconocidos se preservan y no se eliminan.

---

## i18n

Los archivos lang viven en:
