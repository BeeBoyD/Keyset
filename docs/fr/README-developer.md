# Keyset — Référence développeur

Ce document couvre l’architecture, l’organisation des modules, le système de build et le flux de contribution de Keyset.

Pour l’aperçu côté joueur, consultez le [README racine](../../README.md).

---

## Architecture

Keyset est un projet Gradle multimodule. Toute la logique métier vit dans `modules/core` : Java pur, sans imports Minecraft ni loader. Les entrypoints de plateforme restent de simples wrappers.
...
Les builds s’exécutent dans GitHub Actions. Ne lancez pas de builds complets du workspace localement sauf pour déboguer le système de build.
...
Ajoutez un tag `[build:<label>]` au message de commit :
...
Requiert `MODRINTH_TOKEN` en variable d’environnement ou `modrinthToken` dans `gradle-local.properties` (ignoré par git).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` utilisent `jar` au lieu de `remapJar` à cause de conflits de mappings. Voir `jarTaskOverrides` dans le `build.gradle` racine.
...
26.1 est la première version Minecraft non obfusquée. Yarn n’est pas utilisé.
...
La ligne `1.0.x` reçoit uniquement des corrections critiques.
Share, Auto-Switch et l’UI 2.0 ne sont pas rétroportés.
...
Les profils sont stockés dans `config/keybindprofiles.json`. Le format est stable sur la famille de releases prise en charge. Les IDs de keybind inconnus sont préservés.

---

## i18n

Les fichiers lang vivent dans :
