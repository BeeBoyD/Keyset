# Keyset — Справочник разработчика

Этот документ описывает архитектуру, структуру модулей, систему сборки и процесс внесения изменений в Keyset.

Обзор для игроков находится в [корневом README](../../README.md).

---

## Architecture

Keyset — много-модульный Gradle-проект. Вся бизнес-логика находится в `modules/core`: чистый Java без импортов Minecraft или loader. Платформенные entrypoints остаются тонкими wrappers.
...
Сборки выполняются в GitHub Actions. Не запускайте полную сборку workspace локально, кроме случаев отладки системы сборки.
...
Добавьте тег `[build:<label>]` в сообщение commit:
...
Требуется `MODRINTH_TOKEN` как переменная окружения или `modrinthToken` в `gradle-local.properties` (gitignored).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` используют `jar` вместо `remapJar` из-за конфликтов mappings. См. `jarTaskOverrides` в корневом `build.gradle`.
...
26.1 — первая необфусцированная версия Minecraft. Yarn не используется.
...
Линия `1.0.x` получает только критические исправления.
Share, Auto-Switch и UI 2.0 не backport-ятся.
...
Профили хранятся в `config/keybindprofiles.json`. Формат стабилен в поддерживаемой линейке релизов. Неизвестные keybind ID сохраняются.

---

## i18n

Lang-файлы находятся в:
