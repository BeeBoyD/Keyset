# Keyset — Entwicklerreferenz

Dieses Dokument beschreibt Architektur, Modulaufbau, Build-System und Beitragsworkflow von Keyset.

Die Spielerübersicht steht im [Root README](../../README.md).

---

## Architecture

Keyset ist ein Gradle-Multiprojekt. Die gesamte Businesslogik liegt in `modules/core`: reines Java ohne Minecraft- oder Loader-Imports. Plattform-Entrypoints sind nur dünne Wrapper.
...
Builds laufen in GitHub Actions. Führe vollständige Workspace-Builds lokal nur aus, wenn du das Build-System debugst.
...
Füge dem Commit-Message ein `[build:<label>]`-Tag hinzu:
...
Benötigt `MODRINTH_TOKEN` als Environment-Variable oder `modrinthToken` in `gradle-local.properties` (gitignored).

### Remapping Overrides

`platform-forge-1_20_6`, `platform-forge-1_21_1`, `platform-fabric-26_1`, `platform-neoforge-26_1` verwenden wegen Mapping-Konflikten `jar` statt `remapJar`. Siehe `jarTaskOverrides` im Root-`build.gradle`.
...
26.1 ist die erste unobfuskierte Minecraft-Version. Yarn wird nicht verwendet.
...
Die Release-Linie `1.0.x` erhält nur kritische Bugfixes.
Share, Auto-Switch und die 2.0-UI werden nicht zurückportiert.
...
Profile werden in `config/keybindprofiles.json` gespeichert. Das Format bleibt in der unterstützten Release-Familie stabil. Unbekannte Keybind-IDs werden erhalten.

---

## i18n

Lang-Dateien liegen unter:
